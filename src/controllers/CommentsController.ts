      ...
      if (comment) {
        ...
        if (result) {
          ...
          return responseToClient(
            ...
            MESSAGE_COMMENT_SUCCESS
          );
        }
        return responseToClient(
          res,
          StatusCode.CODE_ERROR,
          MESSAGE_COMMENT_DELETED_FAILURE
        );
      } else {
        return responseToClient(
          res,
...
function getInfoComment(
  ...
  let result = (comments = comments.map((comment: TypeComment) => {
    let index = users.findIndex(
      (user: TypeUser) => user._id.toString() == comment.author.toString()
    );
    comment.authorInfo = {
...
function getSubComment(comments: TypeComment[]) {
  let results = comments.map((comment: TypeComment) => {
    let subCmt = comments.filter(
      (subcomment: TypeComment) =>
        subcomment.parentComment != null &&
        subcomment.parentComment.toString() == comment._id.toString()
    );
